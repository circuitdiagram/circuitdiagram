...
namespace CircuitDiagram.IO
{
    public static class CircuitDocumentWriter
    {
        ...
        public static void Write(CircuitDocument document, Stream stream)
        {
            //XmlTextWriter writer = new XmlTextWriter(stream, Encoding.UTF8);
            //writer.Formatting = Formatting.Indented;
            //writer.WriteStartDocument();
            ...
            //int internalIdCounter = 0;
            //Dictionary<string, List<ComponentSource>> componentSourcesByLocation = new Dictionary<string,List<ComponentSource>>();
            //Dictionary<ComponentDescription, string> processedDescriptions = new Dictionary<ComponentDescription, string>();
            //foreach (ComponentDescription description in descriptionsInDocument)
            //{
            //    if (processedDescriptions.ContainsKey(description))
            //        continue;
            ...
            //            if (description.Metadata.Type.ToLower().Contains("cdcom"))
            //                contentType = "application/circuitdiagram.binarycomponentdescription";
            ...
            //                List<ComponentSource> tempSources = new List<ComponentSource>(description.Source.ContainedDescriptions.Count);
            //                byte[] buffer = File.ReadAllBytes(description.Source.Path);
            //                Resource originalFileResource = rc.AddResource(buffer, contentType, Path.GetFileName(description.Source.Path));
            //                foreach (ComponentDescription containedDescription in description.Source.ContainedDescriptions)
            //                {
            //                    ComponentSource newSource = new ComponentSource(originalFileResource, internalIdCounter.ToString(), containedDescription.ID, containedDescription.ComponentName, containedDescription.Metadata.GUID);
            //                    processedDescriptions.Add(containedDescription, newSource);
            //                    tempSources.Add(newSource);
            //                    internalIdCounter++;
            //                }
            //                componentSourcesByLocation.Add(tempSources[0].Resource.Location, tempSources);
            //            }
            //            else
            //            {
            //                // Embed only used description - serialize as binary
            ...
            //                {
            //                    BinaryWriter descriptionWriter = new BinaryWriter(tempStream);
            //                    descriptionWriter.WriteDescriptions(description);
            //                    descriptionWriter.Flush();
            //                    ComponentSource newSource = new ComponentSource(rc.AddResource(tempStream.ToArray(), "application/circuitdiagram.binarycomponentdescription", description.ComponentName + ".cddx"), internalIdCounter.ToString(), "C0", description.ComponentName, description.Metadata.GUID);
            //                    //processedDescriptions.Add(description, newSource);
            //                    tempSources.Add(newSource);
            //                }
            //                internalIdCounter++;
            ...
            //        else
            //        {
            //            ComponentSource newSource = new ComponentSource(null, internalIdCounter.ToString(), null, description.ComponentName, description.Metadata.GUID);
            //            processedDescriptions.Add(description, newSource);
            //            if (!componentSourcesByLocation.ContainsKey("_unknown"))
            //                componentSourcesByLocation.Add("_unknown", new List<ComponentSource>());
            //            componentSourcesByLocation["_unknown"].Add(newSource);
            //            internalIdCounter++;
            //        }
            //    }
            //    else
            ...
            //        ComponentSource newSource = new ComponentSource(null, internalIdCounter.ToString(), null, description.ComponentName, description.Metadata.GUID);
            //        processedDescriptions.Add(description, newSource);
            //        if (!componentSourcesByLocation.ContainsKey("_standard"))
            //                componentSourcesByLocation.Add("_standard", new List<ComponentSource>());
            //            componentSourcesByLocation["_standard"].Add(newSource);
            //        internalIdCounter++;
            //    }
            //}
            ...
            //// Component elements
            //WriteDocumentElements(writer, document, processedDescriptions);
